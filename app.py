...
def _get_system_info_sync() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            'disk': {'total': disk_total, 'used': disk_used, 'percent': disk_percent, 'path': ZOTERO_STORAGE},
            ...
            'temperature': {'cpu': cpu_temp, 'unit': '°C' if cpu_temp is not None else 'N/A'}
        }
...
async def view_file(request):
    ...
    if filename.lower().endswith('.pdf'):
        ...
        return web.Response(text=f'<a href="{epub_url}">Download EPUB</a>', content_type='text/html')
...
async def handle_404(request):
    """404处理"""
    return aiohttp_jinja2.render_template('404.html', request, {'request': request}, status=404)
